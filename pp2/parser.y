...
%type <nullconstant>   NullConstant
%type <call>          Call
...
ClassDecl :   T_Class T_Identifier Extend Impl '{' Fields '}'
          ...
          |   T_Class T_Identifier Extend Impl '{' '}'   { $$ = new ClassDecl(new Identifier(@2, $2), $3, $4, new List<Decl*>); }
          ;
...
Implements :  Implements ',' NamedType  { ($$ = $1)->Append($3); }
...
SwitchStmt    :   T_Switch '(' Expr ')' '{' Cases Default '}'  { $$ = new SwitchStmt($3, $6, $7); }
              ;
...
Case          :   T_Case IntConstant ':' Stmts  { $$ = new CaseStmt($2, $4); }
