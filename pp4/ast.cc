...
FnDecl* Node::GetEnclosFunc(Node* node){
  // Returns the enclosing function node
  // of the passed node, if it exists.
  // Else returns NULL.
...
ClassDecl* Node::GetEnclosClass(Node* node){
  // Returns the node corresponding to the innermost
  // class enclosing the passed node. Returns NULL if
