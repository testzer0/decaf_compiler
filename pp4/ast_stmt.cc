...
Location* Program::Emit(){
    // Emits bytecode for the program
    for(int i = 0; i < this->decls->NumElements(); i++)
        this->decls->Nth(i)->SetLabels(); //set labels
    Decl* main_decl = NULL;
    for(int i = 0; i < this->decls->NumElements(); i++){
        Decl* decl = this->decls->Nth(i);
        if(!strcmp(decl->GetID()->GetName(), "main"))
            main_decl = decl; //We found "main"
        else 
            decl->Emit(); //For all others emit bytecode
    }
    if(main_decl)
        main_decl->Emit(); // Finally emit bytecode for main
    Program::cg->DoFinalCodeGen();
    if(Program::sym_table->Lookup("main") == NULL) //main not defined
...
Location* ForStmt::Emit(){
    //Emits bytecode for the for loop
    if(this->test){
        if(this->init)
            this->init->Emit();
        ...
        //If condition no longer true then goto label_1
        //i.e. break out of loop

        if(this->body)
            this->body->Emit(); //body

        if(this->step)
            this->step->Emit(); //increment etc

        Program::cg->GenGoto(label_0);//loop
...
Location* SwitchStmt::Emit(){
    if(this->expr){
        ...
        //label for break
        this->next = labels[2*num + 1]; //marks end of switch

        //labels from case0 to case(n-1)
        for(int i = 0; i < num; i++){
            ...
            if(fndecl){
                ...
                if(i > 0)
                    Program::cg->GenLabel(labels[i-1]); //marks end of previous case stmt

                if(test){
                    Program::cg->GenIfZ(test, labels[i]); 
                    //if case_value != check_value then jump to end of case
                    //in this way keep jumping through cases until(if) we find the
                    //right one(s)
                    Program::cg->GenGoto(labels[num+i]);
                    //if found then goto case body, at labels_(num+i)
                }
            }
        }
        ...
        Program::cg->GenGoto(labels[num*2]); //default
        ...
        Program::cg->GenLabel(labels[2*num + 1]);
    }
    return NULL;
}
